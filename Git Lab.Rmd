...
Do this now, the sooner we get your Github accounts and set up a working verion of git the earlier we can spot and work through inevitable issues. 
...
You'll notice when you visit the Data Science Labs' [Github page](https://github.com/datasciencelabs-students) you can only see repositores with your github-id on them, you'll get one of these for each homework throughtout the semester. When a new homework is released you can go to the corresponding repository to see what's in store. The workflow will be pretty simple. 
...
Ater writing some of your homework in an `Rmd` file, then `knit` it, make pretty plots, find out some cool stuff about the dataset it's time to `add/commit/push`. After some work if you head back to `Terminal` you'll see that something has changed when you type `git status`:
...
Now you'll notice that the files have turned green and are now labeled as changes to be commited, now it's time to commit. This is equivalent to `save` in most programs. But what's special about `git` and other version control software is that we can track and revert changes! We also need to give what's called a `commit message` which will help us keep track of the changes we made when we loot at this in the future. Leave deteailed messages so that future you will know what you did. Future you will thank you. We'll get to this part later. Notice the `-am` flag, the `a` stands for *all*, as in all tracked files, and the `m` stands for *message*.
